...
void Map::PrintEssentialGraph()
{
    ...
    vector<KeyFrame *> vpOriginKFs = mvpKeyFrameOrigins;  // 这里面只会有0个或者1个，保存的是初始话的第一帧
    int count = 0;
...
/**
 * @brief 检查essentialgraph 地图里面所有关键帧应该都会有父帧与子帧（除了头尾）。
 * 如果最后count与mspKeyFrames数量不一致，表示essentialgraph有问题
 */
bool Map::CheckEssentialGraph()
{
    vector<KeyFrame *> vpOriginKFs = mvpKeyFrameOrigins;  // TODO 这里面只会有0个或者1个，保存的是初始化的第一帧，不清楚融合时会有多少
    cout << "Number of origin KFs: " << vpOriginKFs.size() << endl;
    KeyFrame *pFirstKF;
    // 1. 在当前地图中找到初始化那几帧中的父帧，也就是最顶的那帧
    for (KeyFrame *pKFi : vpOriginKFs)
    {
        if (!pFirstKF)
            pFirstKF = pKFi;
    ...
    // TODO 留个疑问，一个关键帧的子节点是否全部在这个帧对应的map里面？？？？？？？？？？？？？？？？？？？？？？？？
    // 3. 将每个子关键帧的子关键帧全部加入到vpChilds
    // 数量限制到mspKeyFrames.size() + 10 如果上述的结果是是，那这个限制条件不会触发
    int count = 0;
    for (int i = 0; i < vpChilds.size() && count <= (mspKeyFrames.size() + 10); ++i)
    {
        count++;
    ...
    // 4. 地图里面所有关键帧应该都会有父帧与子帧（除了头尾），如果最后count与mspKeyFrames数量不一致，表示essentialgraph有问题
