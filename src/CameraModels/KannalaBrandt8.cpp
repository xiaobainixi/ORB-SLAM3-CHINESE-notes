 ...
 * r^2 = xc^2 + yc^2
 * θ = arctan(r)
 * θd = k0*θ + k1*θ^3 + k2*θ^5 + k3*θ^7 + k4*θ^9  k0 可以等于1
 * xd = θd/r * xc   yd = θd/r * yc
 * u = fx*xd + cx  v = fy*yd + cy
 * @param v3D 三维点
 * @return 像素坐标
...
{
    const double x2_plus_y2 = v3D[0] * v3D[0] + v3D[1] * v3D[1];  // x^2 + y^2
    const double theta = atan2f(sqrtf(x2_plus_y2), v3D[2]);  // θ
    const double psi = atan2f(v3D[1], v3D[0]);  // arctan(yc / xc)
    ...
    const double theta_d = theta + mvParameters[4] * theta3 + mvParameters[5] * theta5 + mvParameters[6] * theta7 + mvParameters[7] * theta9;  // θd

    Eigen::Vector2d res;
    res[0] = mvParameters[0] * theta_d * cos(psi) + mvParameters[2];  // cos(psi) = xc / r
    res[1] = mvParameters[1] * theta_d * sin(psi) + mvParameters[3];  // sin(psi) = yc / r
...
cv::Point3f KannalaBrandt8::unproject(const cv::Point2f &p2D)
{
    ...
    float scale = 1.f;
    float theta_d = sqrtf(pw.x * pw.x + pw.y * pw.y);  // sqrt(xc^2 + yc^2)/r * θd
    ...
    if (theta_d > 1e-8)
    {
        ...
        for (int j = 0; j < 10; j++)
        {
            ...
            float k1_theta2 = mvParameters[4] * theta2, k2_theta4 = mvParameters[5] * theta4;
            float k3_theta6 = mvParameters[6] * theta6, k4_theta8 = mvParameters[7] * theta8;
...
cv::Mat KannalaBrandt8::projectJac(const cv::Point3f &p3D)
{
    ...
    // 求解θd 与 θd`
    float f = theta + theta3 * mvParameters[4] + theta5 * mvParameters[5] + theta7 * mvParameters[6] + theta9 * mvParameters[7];
    float fd = 1 + 3 * mvParameters[4] * theta2 + 5 * mvParameters[5] * theta4 + 7 * mvParameters[6] * theta6 + 9 * mvParameters[7] * theta8;

    cv::Mat Jac(2, 3, CV_32F);
    Jac.at<float>(0, 0) = mvParameters[0] * (fd * p3D.z * x2 / (r2 * (r2 + z2)) + f * y2 / r3);  // ∂u/∂x
    Jac.at<float>(1, 0) = mvParameters[1] * (fd * p3D.z * p3D.y * p3D.x / (r2 * (r2 + z2)) - f * p3D.y * p3D.x / r3);  // ∂v/∂x

    Jac.at<float>(0, 1) = mvParameters[0] * (fd * p3D.z * p3D.y * p3D.x / (r2 * (r2 + z2)) - f * p3D.y * p3D.x / r3);  // ∂u/∂y
...
/** 
 * @brief 极线约束，并不是传统意义上的极线约束，但是类似，通过看能否恢复成三维点
 * @param pCamera2 右相机
 * @param kp1 左相机特征点
 * @param kp2 右相机特征点
 * @param R12 2->1的旋转
 ...
 * @return 点的深度值
 */
float KannalaBrandt8::TriangulateMatches(GeometricCamera *pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &R12,
                                         const cv::Mat &t12, const float sigmaLevel, const float unc, cv::Mat &p3D)
{
    // 1. 得到对应特征点的相平面坐标
...
/** 
 * @brief ORBmatcher::SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
                                        vector<pair<size_t, size_t>> &vMatchedPairs, const bool bOnlyStereo, vector<cv::Mat> &vMatchedPoints)
    在这里调用，但是这个函数没用到，暂时不看了
 */
